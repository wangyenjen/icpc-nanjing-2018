/**
 * Choose centroid and 10 largest degrees for p1.
 *
 * [a] \
 * [a] - 1 - 2 - [b]
 * [a] /
...
/**
 * Get m largest degrees in the tree
 */
std::vector<int> get_largest_degree(int n, int m) {
    m = std::min(n, m);
...
int main() {
    ...
    for (int p1 : get_largest_degree(n, 10)) {
        int ans2 = n;
        for (int b0 : adj[p1])
            ans2 = std::min(ans2, solve1(b0, p1, get_size(p1, b0)));
        ans = std::max(ans, ans2);
